"""Render local Blender OSCR demos from saved GNN layout JSON records.
...
transparent cuboids with explicit edge cylinders.
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--face-alpha", type=float, default=0.035)
...
def _make_face_material(name: str, color: tuple[float, float, float], alpha: float) -> bpy.types.Material:
...
def _cube_corners(center: Vector, dims: Vector) -> list[Vector]:
    hx, hy, hz = dims.x * 0.5, dims.y * 0.5, dims.z * 0.5
    return [
        center + Vector((sx * hx, sy * hy, sz * hz))
        for sx in (-1.0, 1.0)
        for sy in (-1.0, 1.0)
        for sz in (-1.0, 1.0)
    ]
...
def _setup_scene(args: argparse.Namespace) -> None:
    ...
    scene.world.color = (0.015, 0.017, 0.02)
    ...
    plane.data.materials.append(_make_face_material("ground_mat", (0.10, 0.11, 0.12), 0.20))
...
def _render_record(record: dict[str, Any], *, args: argparse.Namespace, index: int) -> dict[str, Any]:
    ...
    for draw_order, slot_index in enumerate(order):
        ...
        cube.data.materials.append(
            _make_face_material(f"face_mat_{slot_index}_{label}", color, args.face_alpha)
        )
        edge_mat = _make_emission_material(f"edge_mat_{slot_index}_{label}", color, 1.0)
