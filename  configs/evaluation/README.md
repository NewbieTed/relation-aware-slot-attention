...
The relation-aware generator appends a neutral full-scene background prompt by
default, because the OSCR condition itself is an object-only white canvas. This
helps the conditioned path keep vanilla FLUX's tendency to fill in a plausible
environment. Generation-only prompt add-ons can be overridden without changing
the relation graph used by the GNN:
