...
Because the OSCR condition is an object-only white canvas, the relation-aware
generator adds a neutral full-scene background prompt by default so outputs keep
vanilla FLUX's tendency to synthesize a plausible environment. Override
`--background-prompt`, `--generation-scene-prefix`, `--style-prompt`, or
`--quality-prompt` when a run needs a specific setting.
