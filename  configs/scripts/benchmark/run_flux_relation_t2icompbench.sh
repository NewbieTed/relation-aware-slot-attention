...
GENERATION_SCENE_PREFIX="${GENERATION_SCENE_PREFIX:-}"
...
if [[ -n "$CONFIG_FILE" ]]; then
  ...
  if [[ -n "$GENERATION_PROMPT_SUFFIX" ]]; then
    GEN_CMD+=(--generation-prompt-suffix "$GENERATION_PROMPT_SUFFIX")
  fi
  if [[ -n "$GENERATION_SCENE_PREFIX" ]]; then
    GEN_CMD+=(--generation-scene-prefix "$GENERATION_SCENE_PREFIX")
  fi
  if [[ -n "$BACKGROUND_PROMPT" ]]; then
    GEN_CMD+=(--background-prompt "$BACKGROUND_PROMPT")
  fi
  if [[ -n "$STYLE_PROMPT" ]]; then
    GEN_CMD+=(--style-prompt "$STYLE_PROMPT")
  fi
  if [[ -n "$QUALITY_PROMPT" ]]; then
    GEN_CMD+=(--quality-prompt "$QUALITY_PROMPT")
  fi
fi
